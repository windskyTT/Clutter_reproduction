...
class ClutterEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # clone_environments 会把 env_0 下的资产复制到所有并行环境。
    ...
    def _init_runtime_buffers(self):
        ...
        self.object_pcl_buf = torch.empty((self.num_envs, pcl_points, 3), dtype=torch.float, device=self.device)
    ...
    def generate_reaching_plan_idx(self, env_ids: Sequence[int] | torch.Tensor, actions: torch.Tensor | None = None):
        ...
        旧版使用 `batch_linear_interpolate_poses()`。这里保留同样语义，但实现为
        IsaacLab/wxyz 兼容的 torch 插值。
        """
        ...
        if self.cfg.randomize_tracking_reference:
            self._refresh_current_tracking_reference()
            rand_xyz = sample_uniform(-1.0, 1.0, (len(env_ids), 3), self.device)
            offset_scale = torch.tensor(
                self.cfg.randomize_tracking_reference_range[:3], dtype=torch.float, device=self.device
            )
            self.current_tracking_reference["wrist_initobj_pos"][env_ids] += (rand_xyz * offset_scale).unsqueeze(1)
        ...
        distance = torch.linalg.norm(target_pose[:, :3] - start_pose[:, :3], dim=-1)
        max_step = max(0.04 * float(self.cfg.interpolation_step_scale), 1.0e-4)
        plan_steps = torch.ceil(distance / max_step).to(torch.long).clamp(min=1, max=self.max_episode_length - 1)

        full_plan = linear_interpolate_poses(start_pose, target_pose, self.max_episode_length)
        self.reaching_plan_ee[env_ids] = full_plan
        self.reaching_plan_timesteps[env_ids] = plan_steps
    ...
    def compute_required_observations(self, obs_buf: torch.Tensor, obs_type: str, num_obs: int):
        ...
        if "eefpose" in obs_type:
            obs_buf[:, obs_end : obs_end + 7] = self.eef_pose
            obs_end += 7
        ...
        if "palmpose" in obs_type:
            obs_buf[:, obs_end : obs_end + 7] = self.palm_pose
            obs_end += 7
